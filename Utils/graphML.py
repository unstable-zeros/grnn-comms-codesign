...
LSIGF_t: Applies the same linear shift-invariant graph filter to a sequence of
    graph signals
NVGF_t: Applies a node-variant graph filter to a sequence of graph signals
...
TimeGatedHiddenState: Hidden state of the GRNN with time gating
NodeGatedHiddenState: Hidden state of the GRNN with node gating
EdgeGatedHiddenState: Hidden state of the GRNN with edge gating
...
GraphFilter_t: Creates a graph convolutional layer using LSI graph filters
    that are applied to a time sequence of graph signals (graph process)
NodeVariantGF_t: Creates a graph filtering layer using node-variant graph 
    filters, applied to a sequence of graph signals
...
#############################################################################
#                                                                           #
#                FUNCTIONALS (sequence of graph signals)                    #
#                                                                           #
#############################################################################

def LSIGF_t(h, S, x, b=None):
    """
    LSIGF_t(filter_taps, GSO, input, bias=None) Computes the output of a linear
        shift-invariant graph filter on a sequence of inputs, assuming no
        communication delayes, i.e. assuming S^k x_t is possible.
    ...
    # Now, we have x in B x T x G x N and S in E x N x N, and we want to come up
    # with matrix multiplication that yields z = x * S with shape
    # B x T x E x K x G x N.
    # For this, we first add the corresponding dimensions
    x = x.reshape([B, T, 1, G, N])
    S = S.reshape([1, 1, E, N, N])
    z = x.reshape([B, T, 1, 1, G, N]).repeat(1, 1, E, 1, 1, 1) # This is for k = 0
    # We need to repeat along the E dimension, because for k=0, S_{e} = I for
    # all e, and therefore, the same signal values have to be used along all
    # edge feature dimensions.
    for k in range(1,K):
        x = torch.matmul(x, S) # B x T x E x G x N
        xS = x.reshape([B, T, E, 1, G, N]) # B x T x E x 1 x G x N
        z = torch.cat((z, xS), dim = 3) # B x T x E x k x G x N
    # This output z is of size B x T x E x K x G x N
    # Now we have the x*S_{e}^{k} product, and we need to multiply with the
    # filter taps.
    # We multiply z on the left, and h on the right, the output is to be
    # B x T x N x F (the multiplication is not along the N dimension), so we reshape
    # z to be B x T x N x E x K x G and reshape it to B x T x N x EKG (remember we
    # always reshape the last dimensions), and then make h be E x K x G x F and
    # reshape it to EKG x F, and then multiply
    y = torch.matmul(z.permute(0, 1, 5, 2, 3, 4).reshape([B, T, N, E*K*G]),
                     h.reshape([F, E*K*G]).permute(1, 0)).permute(0, 1, 3, 2)
    # And permute againt to bring it from B x T x N x F to B x T x F x N.
    # Finally, add the bias
    if b is not None:
        y = y + b
    return y

def NVGF_t(h, S, x, b=None):
    """
    NVGF_t(filter_taps, GSO, input, bias=None) Computes the output of a
    node-variant graph filter on a sequence of graph signals and then adds bias.
    It is assumed that there are no communication delays.
    ...
    #                                                             x number_nodes
    # S is edge_weighs x number_nodes x number_nodes
    # x is batch_size x time_samples x input_features x number_nodes
    # b is output_features x number_nodes
    # Output:
    # y is batch_size x time_samples x output_features x number_nodes
    ...
    # B x T x E x K x G x N.
    # For this, we first add the corresponding dimensions
    xr = x.reshape([B, T, 1, G, N])
    Sr = S.reshape([1, 1, E, N, N])
    z = xr.reshape([B, T, 1, 1, G, N]).repeat(1, 1, E, 1, 1, 1) # This is k = 0
    # We need to repeat along the E dimension, because for k=0, S_{e} = I for
    # all e, and therefore, the same signal values have to be used along all
    # edge feature dimensions.
    for k in range(1,K):
        xr = torch.matmul(xr, Sr) # B x T x E x G x N
        xS = xr.reshape([B, T, E, 1, G, N]) # B x T x E x 1 x G x N
        z = torch.cat((z, xS), dim = 2) # B x T x E x k x G x N
    # This output z is of size B x T x E x K x G x N
    # Now we have the x*S_{e}^{k} product, and we need to multiply with the
    # filter taps.
    # This multiplication with filter taps is ``element wise'' on N since for
    # each node we have a different element
    # First, add the extra dimension (F for z, and B for h)
    z = z.reshape([B, T, 1, E, K, G, N])
    h = h.reshape([1, 1, F, E, K, G, N])
    # Now let's do elementwise multiplication
    zh = z * h
    # And sum over the dimensions E, K, G to get B x T x F x N
    y = torch.sum(zh, dim = 5) # Sum over G
    y = torch.sum(y, dim = 4) # Sum over K
    y = torch.sum(y, dim = 3) # Sum over E
    # Finally, add the bias
    if b is not None:
        y = y + b
    return y
...
#############################################################################
#                                                                           #
#            LAYERS (Filtering time-varying signals, fixed GSO)             #
#                                                                           #
#############################################################################

class GraphFilter_t(nn.Module):
    """
    GraphFilter_t Creates a (linear) layer that applies a graph filter (i.e. a
        graph convolution) considering trajectories of time signals, with no
        communication delays and for a fixed GSO

    Initialization:

        GraphFilter_t(in_features, out_features, filter_taps,
                       edge_features=1, bias=True)
    ...
    Add graph shift operator:
        ...
        GraphFilter_t.addGSO(GSO) Before applying the filter, we need to define
        the GSO that we are going to use. This allows to change the GSO while
        using the same filtering coefficients (as long as the number of edge
        features is the same; but the number of nodes can change).
    ...
    def forward(self, x):
        ...
        assert x.shape[3] == self.N
        # Compute the filter output
        u = LSIGF_t(self.weight, self.S, x, self.bias)
        # u is of shape batchSize x time x dimOutFeatures x numberNodes
        return u
...
class NodeVariantGF_t(nn.Module):
    """
    NodeVariantGF_t Creates a filtering layer that applies a node-variant graph
        filter to a sequence of graph signals

    Initialization:

        NodeVariantGF_t(in_features, out_features, shift_taps, node_taps
                    edge_features=1, bias=True)
    ...
    Add graph shift operator:
        ...
        NodeVariantGF_t.addGSO(GSO) Before applying the filter, we need to
        define the GSO that we are going to use. This allows to change the GSO
        while using the same filtering coefficients (as long as the number of
        edge features is the same; but the number of nodes can change).
    ...
    def forward(self, x):
        # x is of shape: batchSize x timeSamples x dimInFeatures x numberNodesIn
        assert len(x.shape) == 4
        assert x.shape[3] == self.N
        # If we have less filter coefficients than the required ones, we need
        # to use the copying scheme
        if self.M == self.N:
            ...
            # C = torch.zeros((self.M, self.N), device = self.copyNodes.device)
            # for i in range(self.N):
            #     C[self.copyNodes[i], i] = 1.
            # self.h = torch.matmul(self.weight,C)
            h = torch.index_select(self.weight, 4, self.copyNodes)
        # Compute the filter output
        u = NVGF_t(h, self.S, x, self.bias)
        # u is of shape batchSize x timeSamples x dimOutFeatures x numberNodes
        return u
