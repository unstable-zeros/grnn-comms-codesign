...
class LQR(_data):
    ...
    def __init__(self, nTrain, nValid, nTest, T, A, B, Q, R, x0 = None,
                 dataType = np.float64, device = 'cpu'):
        # Initialize parent
        super().__init__()
        # store attributes
    ...
    def computeRadiusOfAttraction(self, **kwargs):
        # We generate random signals with predefined energy values and see
        # if they go to zero or not.
        ...
        if 'initRadius' in kwargs.keys():
            initRadius = kwargs['initRadius']
        else:
            initRadius = 1.
            
        if 'radiusProg' in kwargs.keys():
            radiusProg = kwargs['radiusProg']
            assert radiusProg == 'sum' or radiusProg == 'prod'
        else:
            radiusProg = 'sum'
        ...
        if 'radiusChange' in kwargs.keys():
            radiusChange = kwargs['radiusChange']
        else:
            radiusChange = 10.
            
        if 'maxRadius' in kwargs.keys():
            maxRadius = kwargs['maxRadius']
        else:
            maxRadius = 1000.
            
        if doSum:
            assert radiusChange > 0.
        else:
            assert radiusChange > 1.
            
        if 'zeroTolDeriv' in kwargs.keys():
            zeroTolDeriv = kwargs['zeroTolDeriv']
        else:
            zeroTolDeriv = 1e-3
        ...
        if 'saveDir' in kwargs.keys():
            saveDirFigs = os.path.join(kwargs['saveDir'],'figs')
            if not os.path.exists(saveDirFigs):
                os.makedirs(saveDirFigs)
            saveDirFigs = os.path.join(saveDirFigs, 'unstableTrajectories')
            if not os.path.exists(saveDirFigs):
                os.makedirs(saveDirFigs)
            doFigs = True
        else:
        ...
        if 'modelName' in kwargs.keys():
            if doFigs:
                saveDirFigs = os.path.join(saveDirFigs, kwargs['modelName'])
                if not os.path.exists(saveDirFigs):
                    os.makedirs(saveDirFigs)
            
        
        # Compute the trajectory, see if at last one of them violates, and
        # return that
        stable = True
        radius = initRadius
        while stable:
            ...
            # Force it to have the appropriate energy
            energy = np.linalg.norm(x0, axis = 1)
            x0 = x0 / np.expand_dims(energy, 1) * radius
            # Compute the trajectory
            x, _= self.computeTrajectory(x0 = x0, controller = controller)
            # This trajectory is of shape B x T x N
            # Let's compute the norm
            x = np.linalg.norm(x, axis = 2) # B x T
            # and get the derivative of the norm as a function of time
            dx = x[:, 1:] - x[:, :-1] # B x (T-1)
            # Compute the mean value of the derivative for the last half of the
            # trajectory
            dxMean = np.mean(dx[:,round(T/2):], axis = 1) # B
            # And check if it is negative for all elements in the sample
            if np.sum(dxMean < zeroTolDeriv) / B >= prob:
                # If it's negative for all values of the batch, then the
                # system is stable and we have to increase the radius to
                # try to get it out of stability
                if doSum:
                    radius = radius + radiusChange
                else:
                    radius = radius * radiusChange
                
                # If we exceed some radius and it is still stable, then
                # we assume that the radius is infinite
                if radius > maxRadius:
                    stable = False
                    radius = np.inf
            else:
                # We found that it is not stable
                stable = False
                # So we want to plot the trajectories that violate this, just to
                # see what happens
                if doFigs:
                    unstableTrajectories = np.nonzero(dxMean >= zeroTolDeriv)[0]
                    for i in unstableTrajectories:
                        trajectoryFig = plt.figure(figsize = (1.61*5, 1.*5))
                        plt.plot(np.arange(0,T), x[i,:],
                                 color = '#003262', linewidth = 2)
                        plt.xlabel(r'$t$', fontsize = 14)
                        plt.ylabel(r'$\|\mathbf{x}(t)\|$', fontsize = 14)
                        plt.xticks(fontsize = 12)
                        plt.yticks(fontsize = 12)
                        trajectoryFig.savefig(os.path.join(
                                                       saveDirFigs,
                                                      'trajectory%03d.pdf' % i),
                                              bbox_inches = 'tight')
                        plt.close(trajectoryFig)
        
        return radius
    
    def computeL2gain(self, **kwargs):
        # This is how we will do it now:
        # We're going to compute initial points of increasing size, from
        # initRadius to maxRadius with radiusProg/radiusChange determining
        # the in-between points.
        
        # We're going to compute the trajectories for the given controller, 
        # and we're going to compute the L2 gain of the system.
        # We're going to return the maximum of those.
        ...
        #   Force it to have the appropriate energy
        energy = np.linalg.norm(x0, axis = 1)
        x0 = x0 / np.expand_dims(energy, 1) * initRadius
        #   Compute the trajectory
        x, u= self.computeTrajectory(x0 = x0, controller = controller)
        #   This trajectory is of shape B x T x N
        #   Let's compute the norm
        x = np.linalg.norm(x, axis = 2) # B x T
        u = np.linalg.norm(u, axis = 2) # B x T
        #   And add it over all t
        x = np.sum(x, axis = 1) # B
        u = np.sum(u, axis = 1) # B
        #   Average over the batch and save
        return np.median(x/u)
