...
def run(filename, N, degree, T, p, q, h, A_norm, B_norm, threshold, num_epoch,
        batch_size, ensemble_size, val_size, grnn_hidden_dim,
        gcnn_decay_interval, num_topologies, num_x0s, verbose, device,
        log_interval, validation_size, grnn_criterion, gcnn_criterion,
        sparse_criterion):
    ...
    for counter in range(num_topologies):
        ...
        gcnn_optimizer = optim.Adam(gcnn_model.parameters(), lr=0.01,
                                    weight_decay=2)
        gcnn_scheduler = optim.lr_scheduler.StepLR(gcnn_optimizer, 1, 0.9)
        ...
        #print('Sparse edges: ', num_sparse_edges[counter])
        model_params['S_trainable'] = True
