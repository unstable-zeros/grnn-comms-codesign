...
class Model:
    ...
    def __init__(self,
        ...
        # Check penalties
        #   Check if any penalty multiplier has been specified
        if len(penaltyMultiplier) > 0:
            # Check that as many penalty functions as multipliers
            assert len(penaltyMultiplier) == len(penaltyFunction)
            self.penalty = True
    ...
    def __repr__(self):
        ...
        if self.penalty:
            ...
            reprString += "----------\n"
            reprString += "\n"
            for p in range(len(self.penaltyMultiplier)):
                reprString += repr(self.penaltyFunction[p])
                reprString += " (%.4f)" % self.penaltyMultiplier[p]
                reprString += "\n"
        return reprString
