...
TrainerLQRimitation: trainer class that computes a loss over the training set,
    suited for the problem of LQR, assuming that training is being done by means
    of imitation learning (i.e. a controller to be imitated needs to be 
    provided)
TrainerLQRself: trainer class that computes a loss over the training set,
    suited for the problem of LQR, assuming that training is being done in a
    self-supervised manner (by directly minimizing the cost function)
"""
...
class TrainerLQRself(Trainer):
    """
    TrainerLQRself: trainer class that computes a loss over the training set,
        suited for the problem of LQR, assuming that training is being done in
        a self-supervised manner (by directly minimizing the cost function)
    ...
    def __init__(self, model, data, nEpochs, batchSize, **kwargs):
        ...
        super().__init__(model, data, nEpochs, batchSize, **kwargs)
        # I could use this to either handle imitation learning or
        # self-supervised, by picking either one trainBatch function or the 
        # other
        
    def trainBatch(self, thisBatchIndices):
        
        # Get the samples
        xTrain, uTrain = self.data.getSamples('train', thisBatchIndices)
        xTrain = xTrain.to(self.model.device)
        uTrain = uTrain.to(self.model.device)
        ...
        xHatTrain, uHatTrain = self.data.computeTrajectory(
                                                x0 = xTrain[:,0,:,:].squeeze(1),
                                                controller = self.model.archit)
        
        # Compute the cost of that trajectory
        costTrain = self.data.evaluate(xHatTrain, uHatTrain)  
        if self.model.penalty:
            penaltyValue = torch.tensor(0., device = self.model.device)
            for p in range(len(self.model.penaltyMultiplier)):
                thisPenaltyValue = self.model.penaltyFunction[p](self.model,
                                                                 self.data)
                penaltyValue = penaltyValue + \
                             self.model.penaltyMultiplier[p] * thisPenaltyValue
            costTrain = costTrain + penaltyValue          
        ...
        # Since the evaluation cost and the training loss function is the same,
        # we return it twice.
        return costTrain.item(), costTrain.item(), timeElapsed
        
    def validationStep(self):
        
        # Validation:
        xValid, uValid = self.data.getSamples('valid')
        xValid = xValid.to(self.model.device)
        uValid = uValid.to(self.model.device)
        
        # Start measuring time
        startTime = datetime.datetime.now()
        
        # Compute the cost before anything else, since we don't want to update
        # the architecture parameters, or otherwise would reflect the wrong
        # architecure -i.e. not the same one used to compute the loss-
        with torch.no_grad():
            # Compute the trajectory
            xHatValid, uHatValid = self.data.computeTrajectory(
                                                x0 = xValid[:,0,:,:].squeeze(1),
                                                controller = self.model.archit)
            ...
            #   gradient operation is taken into account here.
            #   (Alternatively, we could use a with torch.no_grad():)
            costValid = self.data.evaluate(xHatValid, uHatValid)
        ...
        # Since the evaluation cost and the training loss function is the same,
        # we return it twice.
        return costValid.item(), costValid.item(), timeElapsed

class TrainerLQRstability(TrainerLQRself):
    """
    TrainerLQRstability: trainer class that computes a loss over the training
        set, suited for the problem of LQR, assuming that training is being 
        done in a self-supervised manner (by directly minimizing the cost 
        function). Includes a penalty on the stability bound.
    ...
    Initialization:
        ...
        penaltyMultiplier (float): the multiplier for the penalty of the 
            stability bound
    ...
    def __init__(self, model, data, nEpochs, batchSize, **kwargs):
        ...
        assert 'penaltyMultiplier' in kwargs.keys()
        self.penaltyMultiplier = kwargs.pop('penaltyMultiplier')
    ...
    def trainBatch(self, thisBatchIndices):
        ...
        # Compute the penalty
        stabilityPenalty = stabilityConstant(self.data.A, self.data.B,
                                             self.model.archit)
        
        # Train loss
        lossValueTrain = costTrain + self.penaltyMultiplier * stabilityPenalty
...
class TrainerLQRimitation(Trainer):
    """
    TrainerLQRimitation: trainer class that computes a loss over the training
        set, suited for the problem of LQR, assuming that training is being 
        done by means of imitation learning (i.e. a controller to be imitated 
        needs to be provided in the dataset)
    ...
    # In flocking I also define train() because I need to handle DAGger
    # It's not a bad idea to include DAGger here too (every time-dependent), but
    # not for now
    ...
    def trainBatch(self, thisBatchIndices):
        ...
        # Obtain the output of the GNN
        uHatTrain = self.model.archit(xTrain[:, 0:-1, :, :]) # The last element of
        # the sequence is not required, because x_{T} depends on u_{T-1}, so
        # for a trajectory of length T, we only need u up to u_{T-1}. And since
        # we do u_{t} = Phi(x_{t})

        # Compute loss
        lossValueTrain = self.model.loss(uHatTrain, uTrain)
    ...
    def validationStep(self):
        ...
        with torch.no_grad():
            # Obtain the output of the GNN
            uHatValid = self.model.archit(xValid[:,0:-1,:,:])

            # Compute loss
            lossValueValid = self.model.loss(uHatValid, uValid)
